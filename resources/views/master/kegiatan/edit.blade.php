    ...
    <form action="{{ route('master.kegiatan.update', $kegiatan->id_kegiatan) }}" method="POST" class="p-6">
        ...
        <div class="max-w-2xl space-y-6">
            <div>
                ...
                <select id="id_program" name="id_program" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm @error('id_program') border-red-500 @enderror">
                    ...
                    @foreach($programs as $program)
                        <option value="{{ $program->id_program }}" {{ old('id_program', $kegiatan->id_program) == $program->id_program ? 'selected' : '' }}>{{ $program->kode_program ? $program->kode_program.' — ' : '' }}{{ $program->nama_program }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="kode_kegiatan" name="kode_kegiatan" required value="{{ old('kode_kegiatan', $kegiatan->kode_kegiatan) }}" autocomplete="off" placeholder="Contoh: KG.01" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm font-mono @error('kode_kegiatan') border-red-500 @enderror">
