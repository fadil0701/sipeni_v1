...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Tambah Paket Pengadaan</h1>
        <p class="mt-1 text-sm text-gray-600">Buat paket pengadaan barang dan jasa baru</p>
    </div>
...
<div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
    <form action="{{ route('procurement.paket-pengadaan.store') }}" method="POST">
        @csrf
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div class="sm:col-span-2">
                ...
                <select id="id_sub_kegiatan" name="id_sub_kegiatan" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    @foreach($subKegiatanList ?? [] as $sk)
                        <option value="{{ $sk->id_sub_kegiatan }}" {{ old('id_sub_kegiatan') == $sk->id_sub_kegiatan ? 'selected' : '' }}>{{ $sk->nama_sub_kegiatan }} ({{ $sk->kegiatan?->program?->nama_program ?? '-' }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="id_rku" name="id_rku" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    @foreach($rkuList ?? [] as $rku)
                        <option value="{{ $rku->id_rku }}" {{ old('id_rku') == $rku->id_rku ? 'selected' : '' }}>{{ $rku->no_rku }} - {{ $rku->tahun_anggaran }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="no_paket" name="no_paket" value="{{ old('no_paket') }}" required maxlength="100" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" id="nama_paket" name="nama_paket" value="{{ old('nama_paket') }}" required maxlength="255" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="deskripsi_paket" name="deskripsi_paket" rows="3" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('deskripsi_paket') }}</textarea>
            ...
            <div>
                ...
                <select id="metode_pengadaan" name="metode_pengadaan" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    <option value="PEMILIHAN_LANGSUNG" {{ old('metode_pengadaan', 'PEMILIHAN_LANGSUNG') == 'PEMILIHAN_LANGSUNG' ? 'selected' : '' }}>Pemilihan Langsung</option>
                    <option value="PENUNJUKAN_LANGSUNG" {{ old('metode_pengadaan') == 'PENUNJUKAN_LANGSUNG' ? 'selected' : '' }}>Penunjukan Langsung</option>
                    <option value="TENDER" {{ old('metode_pengadaan') == 'TENDER' ? 'selected' : '' }}>Tender</option>
                    <option value="SWAKELOLA" {{ old('metode_pengadaan') == 'SWAKELOLA' ? 'selected' : '' }}>Swakelola</option>
                </select>
            ...
            <div>
                ...
                <input type="number" id="nilai_paket" name="nilai_paket" value="{{ old('nilai_paket', 0) }}" required min="0" step="0.01" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            ...
            <div>
                ...
                <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai') }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                @error('tanggal_mulai')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700">Tanggal Selesai</label>
                <input type="date" id="tanggal_selesai" name="tanggal_selesai" value="{{ old('tanggal_selesai') }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                @error('tanggal_selesai')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div class="sm:col-span-2">
                <label for="keterangan" class="block text-sm font-medium text-gray-700">Keterangan</label>
                <textarea id="keterangan" name="keterangan" rows="2" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('keterangan') }}</textarea>
