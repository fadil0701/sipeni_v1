...
@section('content')
<div class="mb-4">
    <a href="{{ route('transaction.pemakaian-barang.show', $pemakaian->id_pemakaian) }}" class="text-blue-600 hover:text-blue-900 inline-flex items-center">
        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
        </svg>
        Kembali ke Detail Pemakaian
    </a>
</div>

<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Edit Pemakaian Barang</h2>
        <p class="text-sm text-gray-600 mt-1">No. Pemakaian: <span class="font-semibold">{{ $pemakaian->no_pemakaian }}</span></p>
    </div>
    
    <form action="{{ route('transaction.pemakaian-barang.update', $pemakaian->id_pemakaian) }}" method="POST" class="p-6" id="formPemakaian">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        >
                            ...
                            @foreach($unitKerjas as $unitKerja)
                                <option value="{{ $unitKerja->id_unit_kerja }}" {{ old('id_unit_kerja', $pemakaian->id_unit_kerja) == $unitKerja->id_unit_kerja ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($gudangs as $gudang)
                                <option value="{{ $gudang->id_gudang }}" {{ old('id_gudang', $pemakaian->id_gudang) == $gudang->id_gudang ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($pegawais as $pegawai)
                                <option value="{{ $pegawai->id }}" {{ old('id_pegawai_pemakai', $pemakaian->id_pegawai_pemakai) == $pegawai->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            required
                            value="{{ old('tanggal_pemakaian', $pemakaian->tanggal_pemakaian->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        >
                            <option value="DRAFT" {{ old('status_pemakaian', $pemakaian->status_pemakaian) == 'DRAFT' ? 'selected' : '' }}>Draft</option>
                            <option value="DIAJUKAN" {{ old('status_pemakaian', $pemakaian->status_pemakaian) == 'DIAJUKAN' ? 'selected' : '' }}>Diajukan</option>
                        </select>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        >{{ old('alasan_pemakaian', $pemakaian->alasan_pemakaian) }}</textarea>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea 
                            ...
                            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                        >{{ old('keterangan', $pemakaian->keterangan) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div id="detailContainer" class="space-y-4">
                    @foreach($pemakaian->detailPemakaian as $index => $detail)
                        <div class="item-row bg-gray-50 p-4 rounded-lg border border-gray-200">
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                                    ...
                                    >
                                        ...
                                        @foreach($inventories as $inventory)
                                            <option 
                                                ...
                                                {{ old("detail.$index.id_inventory", $detail->id_inventory) == $inventory->id_inventory ? 'selected' : '' }}
                                            >
                                                {{ $inventory->dataBarang->nama_barang ?? '-' }} (Stok: {{ $inventory->qty_input }} {{ $inventory->satuan->nama_satuan ?? '' }})
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="detail[{{ $index }}][id_detail]" value="{{ $detail->id_detail_pemakaian }}">
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input 
                                        ...
                                        readonly
                                        value="{{ $detail->inventory->qty_input ?? 0 }}"
                                    >
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input 
                                        ...
                                        step="0.01"
                                        placeholder="0"
                                        value="{{ old("detail.$index.qty_pemakaian", $detail->qty_pemakaian) }}"
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    >
                                        ...
                                        @foreach($satuans as $satuan)
                                            <option value="{{ $satuan->id_satuan }}" {{ old("detail.$index.id_satuan", $detail->id_satuan) == $satuan->id_satuan ? 'selected' : '' }}>
                                                {{ $satuan->nama_satuan }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input 
                                        ...
                                        value="{{ old("detail.$index.alasan_pemakaian_item", $detail->alasan_pemakaian_item) }}"
                                        class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                    >
                                </div>
                            </div>
                            <div class="mt-2">
                                ...
                                <textarea 
                                    ...
                                    class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                >{{ old("detail.$index.keterangan", $detail->keterangan) }}</textarea>
                            </div>
...
function loadInventoryByGudang(gudangId) {
    // Filter inventory berdasarkan gudang bisa ditambahkan di sini
}
