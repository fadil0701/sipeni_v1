...
<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Tambah Kode Barang</h2>
    </div>
    
    <form action="{{ route('master-data.kode-barang.store') }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div>
                ...
                <select id="id_aset" name="id_aset" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_aset') border-red-500 @enderror">
                    ...
                    @foreach($asets as $aset)
                        <option value="{{ $aset->id_aset }}" {{ old('id_aset') == $aset->id_aset ? 'selected' : '' }}>{{ $aset->nama_aset }}</option>
                    @endforeach
                </select>
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" id="nama_kode_barang" name="nama_kode_barang" required value="{{ old('nama_kode_barang') }}" placeholder="Masukkan nama kode barang" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('nama_kode_barang') border-red-500 @enderror">
