...
<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Tambah Data Barang</h2>
    </div>
    
    <form action="{{ route('master-data.data-barang.store') }}" method="POST" class="p-6" enctype="multipart/form-data">
        @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <select id="id_subjenis_barang" name="id_subjenis_barang" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_subjenis_barang') border-red-500 @enderror">
                            ...
                            @foreach($subjenisBarangs as $subjenisBarang)
                                <option value="{{ $subjenisBarang->id_subjenis_barang }}" {{ old('id_subjenis_barang') == $subjenisBarang->id_subjenis_barang ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="id_satuan" name="id_satuan" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_satuan') border-red-500 @enderror">
                            ...
                            @foreach($satuans as $satuan)
                                <option value="{{ $satuan->id_satuan }}" {{ old('id_satuan') == $satuan->id_satuan ? 'selected' : '' }}>{{ $satuan->nama_satuan }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_barang" name="nama_barang" required value="{{ old('nama_barang') }}" placeholder="Masukkan nama barang" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('nama_barang') border-red-500 @enderror">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="3" placeholder="Masukkan deskripsi barang" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('deskripsi') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="foto_barang" name="foto_barang" value="{{ old('foto_barang') }}" placeholder="Masukkan URL foto barang" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
