...
<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Edit Kategori Barang</h2>
    </div>
    
    <form action="{{ route('master-data.kategori-barang.update', $kategoriBarang->id_kategori_barang) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select id="id_kode_barang" name="id_kode_barang" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_kode_barang') border-red-500 @enderror">
                    ...
                    @foreach($kodeBarangs as $kodeBarang)
                        <option value="{{ $kodeBarang->id_kode_barang }}" {{ old('id_kode_barang', $kategoriBarang->id_kode_barang) == $kodeBarang->id_kode_barang ? 'selected' : '' }}>
