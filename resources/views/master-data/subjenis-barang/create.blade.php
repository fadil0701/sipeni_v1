...
<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Tambah Subjenis Barang</h2>
    </div>
    
    <form action="{{ route('master-data.subjenis-barang.store') }}" method="POST" class="p-6">
        @csrf
            ...
            <div>
                ...
                <select id="id_jenis_barang" name="id_jenis_barang" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_jenis_barang') border-red-500 @enderror">
                    ...
                    @foreach($jenisBarangs as $jenisBarang)
                        <option value="{{ $jenisBarang->id_jenis_barang }}" {{ old('id_jenis_barang') == $jenisBarang->id_jenis_barang ? 'selected' : '' }}>
