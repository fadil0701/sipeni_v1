...
<div class="bg-white shadow-sm rounded-lg border border-gray-200">
    <div class="px-6 py-5 border-b border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900">Edit Jenis Barang</h2>
    </div>
    
    <form action="{{ route('master-data.jenis-barang.update', $jenisBarang->id_jenis_barang) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select id="id_kategori_barang" name="id_kategori_barang" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('id_kategori_barang') border-red-500 @enderror">
                    ...
                    @foreach($kategoriBarangs as $kategoriBarang)
                        <option value="{{ $kategoriBarang->id_kategori_barang }}" {{ old('id_kategori_barang', $jenisBarang->id_kategori_barang) == $kategoriBarang->id_kategori_barang ? 'selected' : '' }}>
