...
use Database\Factories\PermintaanBarangFactory;
...
{
    /** @use HasFactory<PermintaanBarangFactory> */
    use HasFactory;
    ...
    /**
     * Keep compatibility with legacy uppercase values stored in DB
     * while exposing status as enum to application code.
     */
    protected function status(): Attribute
    {
        return Attribute::make(
            get: fn ($value) => PermintaanBarangStatus::normalizeStored(
                $value !== null ? (string) $value : null
            ),
            set: fn ($value) => [
                'status' => $value instanceof PermintaanBarangStatus
                    ? $value->value
                    : PermintaanBarangStatus::normalizeStored((string) $value)->value,
            ],
        );
    }
